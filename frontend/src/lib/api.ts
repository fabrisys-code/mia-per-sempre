...
import { getClientApiUrl, API_PREFIX } from './config';

// URL base per le chiamate API (calcolato dinamicamente)
const getApiBaseUrl = () => getClientApiUrl() + API_PREFIX;
...
async function apiRequest<T>(
  ...
  if (!response.ok) {
    throw new ApiError(data.detail || data.message || "Errore API", response.status);
  }
...
const api = {
  ...
  images: {
    async upload(propertyId: number, files: File[]) {
      ...
      return uploadRequest<any>(`/images/${propertyId}`, formData);
    },

    async list(propertyId: number) {
      return apiRequest<any>(`/images/${propertyId}`);
    },

    async delete(propertyId: number, imageId: number) {
      return apiRequest<void>(`/images/${propertyId}/${imageId}`, {
        method: "DELETE",
      });
    },

    async setMain(propertyId: number, imageId: number) {
      return apiRequest<any>(`/images/${propertyId}/${imageId}/cover`, {
        method: "PUT",
      });
    },

    async reorder(propertyId: number, imageIds: number[]) {
      return apiRequest<any>(`/images/${propertyId}/reorder`, {
        method: "PUT",
